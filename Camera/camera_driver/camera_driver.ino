...
//this function is the one that seems to stop serial communication from working. or more accurately, the error checking in the twi functions is causing it to crash.
...
//  * This will be on pin 11*/
  ...
  OCR2A = 0;//(F_CPU)/(2*(X+1))
    ...
    /* Setup the 8mhz PWM clock
   * This will be on pin 9
  *  modified by Tim Chia 4/18/2018 to get it on pin 9.  */
  DDRB |= (1 << 1);//pin 9
  ASSR &= ~(_BV(EXCLK) | _BV(AS2)); // AS2 actually affects clock 2, but it's safe to leave this here for now.
